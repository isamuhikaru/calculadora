var calculadora = {
    ...
    plasmar_un_punto: function() {
        ...
        var solo_un_punto = function() {
            if (document.getElementById("display").innerHTML.indexOf('.') < 0) {
                /* indexOf() Devuelve la posición de la primera aparición de un valor especificado en una cadena, en este caso me dice que si el valor de su ubicación es menor a cero pues se pondrá el carácter punto. Este método devuelve -1 si no se encuentra el carácter.
                Así, al dar clic, aun no hay punto y su valor sera -1 osea un numero menor a 0 y por lo tanto se plasmara el punto, al siguiente clic con el botón como ya tendrá un valor el carácter en la cadena, ya no se plasmara el carácter punto */
                var escritura = document.getElementById("display");
                escritura.innerHTML = escritura.innerHTML + this.id;
                //escritura.innerHTML += this.id ;
            };
    ...
    operacion: function() {
        ...
        var valor_primero = document.querySelectorAll("#mas, #menos, #por, #dividido, #raiz");
        ...
        //Este ciclo reparte la funcionalidad a todas las teclas:
        for (var i = 0; i < valor_primero.length; i++) {
            valor_primero[i].addEventListener("click", valor_a);
        };



        var operacion_y_blanco = document.querySelectorAll("#mas, #menos, #por, #dividido, #raiz");
        ...
        //Este ciclo reparte la funcionalidad a todas las teclas:
        for (var i = 0; i < operacion_y_blanco.length; i++) {
            operacion_y_blanco[i].addEventListener("click", opcion);
        };
        ...
        var opcion = function() {
            ...
            //Por si no hay un segundo valor:
            if (segundo_valor == "") {
            ...
            if (document.getElementById("display").innerHTML.length >= 8) {
                ...
                var limite = resultado.slice(0, 8) + "+";
                document.getElementById("display").innerHTML = limite;
            };
        };

        //Este ciclo reparte la funcionalidad a la tecla:
